...
```{r plot metrics, warning=FALSE, fig.cap="Mass by time diagram of all features returned by XCMS. Points have been colored by their calculated peak shape metric and are sized by their calculated signal-to-noise ratio. Large, pale points tend to be higher quality than small dark ones."}
shape_metrics %>%
...
```{r badss, fig.cap="Screenshot of lasso labeling dashboard when 'Bad' features are being lassoed in the principal components graph."}
knitr::include_graphics(file.path(here::here(), "vignettes", "intro_bad_ss.png"), rel_path = FALSE)
```
