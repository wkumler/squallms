...
```{r loadall_squallms, echo=FALSE, include=FALSE}
devtools::load_all()
```
...
saveRDS(xcms_filled, file="vignettes/intro_xcms_filled.rds")
```
...
feat_peak_info <- feat_peak_info %>%
  filter(str_detect(filename, "AB|CD"))
```
...
# saveRDS(manual_classes, "vignettes/intro_manual_labels.rds")
```

```{r load_prior_manual_labels, eval=!interactive(), echo=FALSE}
manual_classes <- readRDS("vignettes/intro_manual_labels.rds")
```
...
# saveRDS(lasso_classes, "vignettes/intro_lasso_labels.rds")
```
...
data.frame(manual=factor(manual_classes), lasso=factor(lasso_classes)) %>%
  filter(!is.na(manual)) %>%
  with(caret::confusionMatrix(lasso, manual, positive="Good"))
```
...
ggsave("intro_model_spread.png", plot = last_plot(), device = "png",
...
```{r, eval=FALSE}
final_xcms <- updateXcmsObjFeats(shape_metrics, class_labels, verbosity = 0, 
